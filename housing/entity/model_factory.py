...
def evaluate_regression_model(model_list: list, X_train:np.ndarray, y_train:np.ndarray, X_test:np.ndarray, y_test:np.ndarray, base_accuracy:float=0.6) -> MetricInfoArtifact:
    ...
    try:
        ...
        for model in model_list:
            ...
            # f1 score is harmonic mean of precision and recall, it givens higher weightage to the lower 
            # acc and lower weightage to the higher acc
            # why iff_test_train_acc ? to check for underfitting and overfitting
            # if train acc is high and test acc is low, overfitting.or it both the acc is low - underfitting
            ...
            if model_accuracy >= base_accuracy and diff_test_train_acc < 0.05:
                ...
                # here we updated base acc to model acc, for eg. base acc is .6 and model acc is .7
                # now base acc is .7 and next model acc to be more than .7 now
            ...
            index_number += 1
            # check for next model and its acc.for finding best model
...
class ModelFactory:
    def __init__(self, model_config_path: str = None,):
        try:
            ...
            #read_params is a static method, so we call this function like.. classname.function()
            ...
            # before training (before calling fit method), what ever model , we have, we will store here.
            self.grid_searched_best_model_list = None
            # after we have performed , grid search , we will store our best model here
    ...
    def read_params(config_path: str) -> dict:
        # this function is declared as static function because, it is not the part of
        #modelfactory class but still it is required, we dont want object of the class to access it
        #it just reads the yaml file and saves as dict.
        try:
    ...
    def class_for_name(module_name:str, class_name:str):
        try:
            ...
            #it is a library in python, for importing modules
    ...
    def get_initialized_model_list(self) -> List[InitializedModelDetail]:
        ...
        try:
            ...
            for model_serial_number in self.models_initialization_config.keys():
                # module_0 and module_1 are the keys - check in cofig/model.yaml
                # now model_serial_numbers are module_0 and module_1

                model_initialization_config = self.models_initialization_config[model_serial_number]
                model_obj_ref = ModelFactory.class_for_name(module_name=model_initialization_config[MODULE_KEY],
                                                            class_name=model_initialization_config[CLASS_KEY]
                                                            )
                # class_for_name is a static method , so it is called by the class itself and not the object
                # for eg. module_name is sklearn.linear_model and class is linearRegression
                # from sklearn.linear_model import linear regression --- this activity is done by class_for_name
                model = model_obj_ref()
    ...
    # List[GridSearchedBestModel] - after search, wht ever object we get with best param , it will return
    # that object
        ...
        try:
            ...
            for initialized_model_list in initialized_model_list:
                ...
                # for one model , it is initiate_best_parameter_search_for_initialized_model function
                #for more than one model , it is initiate_best_parameter_search_for_initialized_models function
